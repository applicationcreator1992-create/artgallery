...
const reshapeProduct = (
  ...
  // NEW: Enhanced variant reshaping with real-time data
  const reshapedVariants = removeEdgesAndNodes(variants).map((variant) => ({
    ...variant,
    // Add real-time inventory fields if available
    inventoryQuantity: variant.inventoryQuantity ?? undefined,
    inventoryManagement: variant.inventoryManagement ?? undefined,
    inventoryPolicy: variant.inventoryPolicy ?? undefined,
    compareAtPrice: variant.compareAtPrice ?? undefined,
  }));

  // NEW: Calculate total inventory from variants
  const totalInventory = reshapedVariants.reduce((total, variant) => {
  ...
  return {
    ...
    // NEW: Add real-time tracking fields
    inventoryTracked: totalInventory > 0,
    totalInventory,
    lastInventoryUpdate: product.updatedAt, // Use product updatedAt as fallback
    lastPriceUpdate: product.updatedAt, // Use product updatedAt as fallback
  };
};
...
export async function revalidate(req: NextRequest): Promise<NextResponse> {
  ...
  // NEW: Add inventory and variant webhooks for real-time updates
  const inventoryWebhooks = [
    "inventory_levels/update",
    "inventory_levels/create",
    "inventory_levels/delete",
  ];
  const variantWebhooks = [
  ...
  const isVariantUpdate = variantWebhooks.includes(topic);
  ...
  if (isCollectionUpdate) {
    ...
    console.log(`🔄 Collection webhook received: ${topic}`);
  }
  ...
  if (isInventoryUpdate) {
    ...
    console.log(`📦 Inventory webhook received: ${topic}`);
  }
